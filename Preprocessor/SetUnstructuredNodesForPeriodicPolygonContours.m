function [INITIAL_POSITIONS,BORDERS,TRIANGULATION]=SetUnstructuredNodesForPeriodicPolygonContours(Contour1,Contour2,Interpolant1,Interpolant2,Average_Nodal_Distance,Mesh_Ratio,Periodic_Boundaries,Activate_Plot)
...
%    end
%end
Contours1=cell(size(Contour1,1)-1,1);
Ncontours1=size(Contours1,1);
for i=1:Ncontours1
    Contours1{i,1}=[Contour1(i,1),Contour1(i,2);Contour1(i+1,1),Contour1(i+1,2)];
end
InterpContour1=[];
for n=1:Ncontours1
    Ltot=sqrt((Contours1{n,1}(2,1)-Contours1{n,1}(1,1))^2+(Contours1{n,1}(2,2)-Contours1{n,1}(1,2))^2);
    Ninterp=ceil(Ltot/(HR*Average_Nodal_Distance*Mesh_Ratio(1)));
    DistInterp=Ltot/Ninterp;
    InterpContour=zeros(Ninterp+1,2);
    InterpContour(1,:)=Contours1{n,1}(1,1:2);
    InterpContour(size(InterpContour,1),:)=Contours1{n,1}(size(Contours1{n,1},1),1:2);
    for i=2:Ninterp
        D=(i-1)*DistInterp;
        ratio=D/Ltot;
        InterpContour(i,:)=(1-ratio)*Contours1{n,1}(1,1:2)+ratio*Contours1{n,1}(2,1:2);
    end
    if isempty(InterpContour1)==0
        InterpContour=InterpContour(2:size(InterpContour,1),:);
    end
    InterpContour1=cat(1,InterpContour1,InterpContour);
end
...
%    end
%end
Contours2=cell(size(Contour2,1)-1,1);
Ncontours2=size(Contours2,1);
for i=1:Ncontours2
    Contours2{i,1}=[Contour2(i,1),Contour2(i,2);Contour2(i+1,1),Contour2(i+1,2)];
end
InterpContour2=[];
for n=1:Ncontours2
    Ltot=sqrt((Contours2{n,1}(2,1)-Contours2{n,1}(1,1))^2+(Contours2{n,1}(2,2)-Contours2{n,1}(1,2))^2);
    Ninterp=ceil(Ltot/(HR*Average_Nodal_Distance*Mesh_Ratio(2)));
    DistInterp=Ltot/Ninterp;
    InterpContour=zeros(Ninterp+1,2);
    InterpContour(1,:)=Contours2{n,1}(1,1:2);
    InterpContour(size(InterpContour,1),:)=Contours2{n,1}(size(Contours2{n,1},1),1:2);
    for i=2:Ninterp
        D=(i-1)*DistInterp;
        ratio=D/Ltot;
        InterpContour(i,:)=(1-ratio)*Contours2{n,1}(1,1:2)+ratio*Contours2{n,1}(2,1:2);
    end
    if isempty(InterpContour2)==0
        InterpContour=InterpContour(2:size(InterpContour,1),:);
    end
    InterpContour2=cat(1,InterpContour2,InterpContour);
end
...
b=fmin-pmin*a;
p1=Contour1(size(Contour1,1),2);
p2=Contour2(1,2);
f1=a*p1+b;
f2=a*p2+b;
%Ninterp=floor(2*(p2-p1)/(f1+f2));
%Dd=(f2-f1)/(Ninterp-1);
...
%end
Contour12(1,1)=Periodic_Boundaries(2);
Contour12(1,2)=p1;
flagout=0;
newY=-1e10;
while newY<p2
    newY=Contour12(size(Contour12,1),2)+0.5*(a*Contour12(size(Contour12,1),2)+b);
    Contour12=cat(1,Contour12,[Periodic_Boundaries(2),newY]);
end
Contour12(:,2)=p1+(Contour12(:,2)-p1)/(newY-p1)*(p2-p1);
Contour12=Contour12(2:size(Contour12,1)-1,:);
...
%Ninterp=ceil(1.2*2*L12/(fmin+fmax));
...
InterpContour=cat(1,InterpContour1,Contour12,InterpContour2,Contour21);
InterpContour=[InterpContour;InterpContour(1,:)];
...
xdist=max(InterpContour(:,1))-min(InterpContour(:,1));
ydist=max(InterpContour(:,2))-min(InterpContour(:,2));
Box=[min(InterpContour(:,1))-xdist/100,min(InterpContour(:,2))-ydist/100;max(InterpContour(:,1))+xdist/100,max(InterpContour(:,2))+ydist/100];
pmin=min(InterpContour(:,2));
pmax=max(InterpContour(:,2));
fmin=Average_Nodal_Distance*Mesh_Ratio(1);
fmax=Average_Nodal_Distance*Mesh_Ratio(2);
a=(fmax-fmin)/(pmax-pmin);
b=fmin-pmin*a;
fh=@(p) b+a*p(:,2);
[INITIAL_POSITIONS,TRIANGULATION]=distmesh2d_plot(Activate_Plot,0.01,@dpoly,fh,0.99*Average_Nodal_Distance,Box,InterpContour,InterpContour);
...
BORDERS={'Periodic',size(Border1,1),Border1,Interpolant1,1,1;'Periodic',size(Border2,1),Border2,Interpolant2,0,0};