function [INITIAL_POSITIONS,BORDERS,TRIANGULATION]=SetManualBordersForSimpleContours(Contours,INITIAL_POSITIONS,TRIANGULATION,Average_Nodal_Distance)
...
% Detect the boundary nodes and segments
Segments=zeros(3*size(TRIANGULATION,1),2);
for i=1:size(TRIANGULATION,1)
...
for i=1:size(Segments,1)
    ...
    for j=1:size(Segments,1)
        ...
        if Segments(j,1)==Segments(i,2) & Segments(j,2)==Segments(i,1)
            FlagSingle=0;
            Done(j)=1;
            break
        end
    ...
    if FlagSingle==1
        Unsorted=[Unsorted;Segments(i,:)];
    end
end

%Affect segments to borders
BORDERS=cell(size(Contours,1),3);
for n=1:size(Contours,1)
    PosIni=Contours{n,2}(1,:);
    PosFin=Contours{n,2}(end,:);
    for i=1:size(Unsorted,1)
        if norm(INITIAL_POSITIONS(Unsorted(i,1),:)-PosIni)<1e-6*Average_Nodal_Distance
            border=Unsorted(i,:)';
            break
        end
    end
    ...
    while FlagOut==0
        for i=1:size(Unsorted,1)
            if Unsorted(i,1)==border(end)
                border(end+1)=Unsorted(i,2);
                break
            end
        end
        if norm(INITIAL_POSITIONS(Unsorted(i,2),:)-PosFin)<1e-6*Average_Nodal_Distance
            FlagOut=1;
            BORDERS{n,1}='Simple';
            BORDERS{n,2}=size(border,1);
            BORDERS{n,3}=border;
