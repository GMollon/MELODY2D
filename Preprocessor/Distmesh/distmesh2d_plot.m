function [p,t]=distmesh2d_plot(Activate_Plot,dptol,fd,fh,h0,bbox,pfix,varargin)
...
ttol=.1; Fscale=1.2; deltat=.2; geps=.001*h0; deps=sqrt(eps)*h0; %dptol=.001;
densityctrlfreq=30;
...
while 1
  ...
  if Activate_Plot==1
      max(sqrt(sum(deltat*Ftot(d<-geps,:).^2,2))/h0)
      count
  end
