function [INITIAL_POSITIONS,BORDERS,TRIANGULATION]=SetUnstructuredNodesForPeriodicContours(Contour1,Contour2,Interpolant1,Interpolant2,Average_Nodal_Distance,Mesh_Ratio,Periodic_Boundaries,Activate_Plot)
...
%b=fmin-pmin*a;
%fh=@(p) b+a*p(:,2);
a=(log10(fmax)-log10(fmin))/(pmax-pmin);
b=log10(fmin)-pmin*a;
fh=@(p) 10.^(b+a*p(:,2));
[INITIAL_POSITIONS,TRIANGULATION]=distmesh2d_plot(Activate_Plot,0.01,@dpoly,fh,0.99*Average_Nodal_Distance,Box,Contour,Contour);
