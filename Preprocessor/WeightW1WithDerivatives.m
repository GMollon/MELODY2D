function W=Weight_W1(Poi_Relative_Positions,List_Domain_Fnodes,Domains_Dimensions)
%
...
%if Domain_Type==1
    for i=1:Number_Fnodes_Domain
        ...
        if r>0.5 & r<=1
            ...
            dwdx=(-4*d^3*x*r+8*d*x^3+8*d*x*y^2-4*x^3*r*d-4*x*y^2*r*d)/(d^3*x^2+d^3*y^2);
            dwdy=(-4*d^3*y*r+8*d*y^3+8*d*y*x^2-4*y^3*r*d-4*y*x^2*r*d)/(d^3*y^2+d^3*x^2);
        elseif r<=0.5
            w=(2/3)-4*r^2+4*r^3;
            %dwdx=(12*x*r-8*x)*dr;
            %dwdy=(12*y*r-8*y)*dr;
            dwdx=(-8*d*x+12*x*sqrt(x^2+y^2))/d^3;
            dwdy=(-8*d*y+12*y*sqrt(x^2+y^2))/d^3;
        else
            w=0;
            dwdx=0;
            dwdy=0;
