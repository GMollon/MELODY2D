function [INITIAL_POSITIONS,BORDERS,TRIANGULATION]=SetRigidNodesForSimpleContours(Contours,Average_Nodal_Distance,Activate_Plot)
...
[INITIAL_POSITIONS,TRIANGULATION]=distmesh2d_plot(Activate_Plot,0.001,@dpoly,@huniform,4*Average_Nodal_Distance,Box,Contour,Contour);
...
for i=1:size(Unsorted,1)
    if norm([INITIAL_POSITIONS(Unsorted(i,1),1)-Contours{1,2}(1,1),INITIAL_POSITIONS(Unsorted(i,1),2)-Contours{1,2}(1,2)])<1e-8*Average_Nodal_Distance
        prev=Unsorted(i,1);
        break
    elseif norm([INITIAL_POSITIONS(Unsorted(i,2),1)-Contours{1,2}(1,1),INITIAL_POSITIONS(Unsorted(i,2),2)-Contours{1,2}(1,2)])<1e-8*Average_Nodal_Distance
        prev=Unsorted(i,2);
        break
    end
end
...
for n=1:Ncontours
    for i=1:size(Unsorted,1)
        if norm([INITIAL_POSITIONS(Unsorted(i,1),1)-Contours{n,2}(size(Contours{n,2},1),1),INITIAL_POSITIONS(Unsorted(i,1),2)-Contours{n,2}(size(Contours{n,2},1),2)])<1e-8*Average_Nodal_Distance
            last=Unsorted(i,1);
            break
        elseif norm([INITIAL_POSITIONS(Unsorted(i,2),1)-Contours{n,2}(size(Contours{n,2},1),1),INITIAL_POSITIONS(Unsorted(i,2),2)-Contours{n,2}(size(Contours{n,2},1),2)])<1e-8*Average_Nodal_Distance
            last=Unsorted(i,2);
            break
        end
    end
    for i=deb:size(Border,1)
        if Border(i,1)==last
            ...
            BORDERS{n,4}=Contours{Ncontours,3};
