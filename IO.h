	...
	while(getline(Static_Control_file, line))
	{
        ...
        if (line.substr(0,12)=="DEACTIVATION")
        {
			...
			for (int i(0) ; i < Nb_deactivated ; i++)
			{
                ...
                if (Keyword == "below")         New_Read.push_back(0.) ;
                else if (Keyword == "equal")    New_Read.push_back(1.) ;
                else if (Keyword == "above")    New_Read.push_back(2.) ;
		...
		if (line.substr(0,7)=="BORDERS")
		{
            ...
            for (int j=0 ; j<nb_borders ; j++) Bodies[index_body].nb_border_nodes += borders[j].number_border_nodes ;
            for (int i(0) ; i < nb_borders ; i++)
			{
			    cout << "Border " << i << endl ;
	...
	while(getline(Static_Data_file, line))
	{
                ...
                for (int i(0) ; i < nb_nodes ; i++)
                {
                    ...
                    if (Bodies[index_body].flag_alid==1)
                    {
                        ...
                        if (xini < Bodies[index_body].alid_xmin + Bodies[index_body].alid_range)
                        {
                            ...
                            if (temp > node.alid_alpha)        node.alid_alpha = temp ;
                            ...
                            if (temp > node.alid_beta)         node.alid_beta = temp ;
                        }
		...
		if (line.substr(0,11)=="INTEGRATION")
		{
            ...
            for (int i=0 ; i<nb_regions ; i++)
            {
                ...
                {
                    if (i*nb_per_region+j<nb_gauss) per_region.push_back(i*nb_per_region+j) ;
                }
            ...
            for (int i=0 ; i<nb_regions ; i++) v[i]=0.;
	...
	vector<int>& flags )
{
	...
	while(getline(Dynamic_file, line))
	{
        ...
        if (line.substr(0,14)=="INITIALIZE_CZM")            flags[3] = 1 ;
        if (line.substr(0,18)=="UPDATE_MASS_MATRIX")        flags[4] = 1 ;
        if (line.substr(0,23)=="UPDATE_STIFFNESS_MATRIX")   flags[5] = 1 ;
...
void Write_monitoring(vector<vector<double>>& monitoring ,
                      vector<int>& flags)
{
	if (flags[7]==0) cout << "Writing monitored data " << endl ;
...
void Write_spying(vector<vector<vector<double>>>& spying ,
                  ...
                  vector<int>& flags)
{
	if (flags[7]==0) cout << "Writing spied data " << endl ;
